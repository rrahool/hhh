...
$(function(){
  ...
  var containerWidth = $('.weekView').width();
  var windowWidth = $(window).width();
  var viewWidth = (containerWidth/windowWidth)*100;

    $('body').on('click','.fc-month-button',function(){
        var cur_month = parseInt($('#current_month').val());
        var cur_year = parseInt($('#current_year').val());
        var cur_time = parseInt($('#current_time').val());
        monthView(cur_month,cur_time,cur_year);
        $('.weekView').hide();
        $('.dayView').hide();
        $('.monthView').show();
    ...
    $('body').on('click','.fc-agendaWeek-button',function(){
        ...
        changeView(cur_daytime, 'weekView');
        $('.dayView').hide();
        $('.monthView').hide();
        $('.weekView').show();
    ...
    function changeView(date, view){
        ...
        $.ajax({
            type: 'post',
            data:{date:date, view:view, courseID:courseID, viewWidth:viewWidth},
            url:"change_view.php",
            success: function(e){
                $('div.'+view).html(e);
                //$('#wait').hide();
    ...
    function weekView(date){
        ...
        setTimeout(function(){
            ...
            $.ajax({
                type: 'post',
                data:{date:date, activities:checkedActivities, courseID:courseID, containerWidth:viewWidth},
                ...
                success: function(e){
                    ...
                    //console.log(columnWidthPx );
                    $('.fc-draggable').draggable({
                        grid: [ columnWidthPx, 61 ],
                        containment:'.container',
                        scroll: false,
                        ...
                        stop: function(event, ui){
                            ...
                            period_from = Math.ceil(period_from);
    ...
    function monthView(month, dateTime, year){
        changeView(dateTime, 'monthView');
        var checkedActivities = activityTypes();
        //alert(checkedActivities);
        $('#wait').show();
        setTimeout(function(){
            //alert(month);
            $.ajax({
                type: 'post',
                data: {month:month, activities:checkedActivities, courseID:courseID, year:year},
                url:"events_array.php",
                dataType:'JSON',
                success: function(e){
                    //alert(e);
                    ...
                    var eventCount=Array();
                    $.each(e, function(key, val){
                      //console.log(val.start_date);
                      eventCount[val.start_date] = 0;
                    });

                    $.each(e, function(key, val){
                        var title = val.title;
                        if(title.length > 25) {
                          //title = title.splice(25, 0, "<br> ");
                          //title = title.substring(0,29)+"..";
                        }
                        var makeHide = "";
                        eventCount[val.start_date]++;
                        if (eventCount[val.start_date] > 20){
                          console.log(eventCount[val.start_date]);
                          makeHide = "hide more-event";
                        }

                        var event1 = '<div class="fc-day-grid-event fc-h-event fc-event fc-start fc-end fc-draggable '+makeHide+'" style="background-color:'+val.bgcolor+'; ">' +
                            // '<div class="fc-content">' +
                            // '<span class="fc-time"></span> ' +
                            // '<div class="fc-title">'+title+'</div></div>' +
                            title+
                    ...
                    $.each(eventCount, function(key, val){
                      key = (key <10)?"0"+key: key;
                      val = eventCount[key];
                      //console.log(key, val);
                      if(val > 3){
                        var more = val - 3;
                        var moreEvent = '<a data-date="'+key+'" class="fc-day-grid-event fc-h-event fc-event fc-start fc-end fc-draggable fc-more" style="background-color:'+val.bgcolor+'; ">' +
                            '<div class="fc-content">' +
                            '<span class="fc-time"></span> ' +
                            '<span class="fc-title">'+more+' More</span></div>' +
                            '</a>';
                        //$('.event-catch-'+key).append(moreEvent);
                      }
                    });
    ...
    $('body').on('click','.next-month',function(){
        ...
        monthView(cur_month,dateTime,cur_year);

        $('#current_month').val(cur_month);
        $('#current_year').val(cur_year);
